...
class AdaptiveObservationSerializer(serializers.ModelSerializer):
    ...
    """
    item_code = serializers.PrimaryKeyRelatedField(source="item.code", read_only=True)
    shop_type = serializers.IntegerField(source='shop_type')
    shop_identifier = serializers.
    """
    # commentarys = ItemCommentary.objects.all()
    # itemcommentary = ItemCommentarySerializer(commentarys, many=True, read_only=True)


    #nr_konnunar = serializers.PrimaryKeyRelatedField(source="survey.code", read_only=True)
    nr_konnunar = serializers.SerializerMethodField()
    itemcode_i_konnun = serializers.PrimaryKeyRelatedField(source="item.code", read_only=True)
    status_obs = serializers.CharField(source="flag")
    shopid = serializers.CharField(source="shop_identifier")
    obs_price = serializers.DecimalField(source="observed_price", decimal_places=2, max_digits=8)
    obs_quantity = serializers.DecimalField(source="observed_quantity", decimal_places=2, max_digits=8)
